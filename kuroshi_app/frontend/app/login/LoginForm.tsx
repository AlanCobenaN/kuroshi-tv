  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRegister) {
      try {
        ...
        const result = await signIn('kuroshi', { token: data.access_token, redirect: false })
  ...
  return (
    <div className="auth-page">
      ...
      <style>{`
        ...
        .password-toggle { position: absolute; right: 0.5rem; background: none; border: none; cursor: pointer; color: var(--text-muted); padding: 0.25rem; display: flex; align-items: center; justify-content: center; line-height: 0; transition: color var(--transition-fast); }
